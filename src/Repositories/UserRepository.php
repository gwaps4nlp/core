...
class UserRepository extends BaseRepository
{
	...
	{
		return $this->model->orderBy('score','desc')->pluck('username','id');
	}
	...
	{
		$this->model->setVisible(['count','period']);
		$list = $this->model
			->select(DB::raw('YEARWEEK(created_at) as period, count(*) as count'))
			->groupBy(DB::raw('YEARWEEK(created_at)'))->get();
		foreach($list as &$user)
			$user->setVisible(['count','period']);
		return $list;
	}
	/**
	 * Return the count of new registrations by week.
	...
	public function countRegistrationsByMonth()
	{
		$this->model->setVisible(['count','period']);
		$list = $this->model
			->select(DB::raw("DATE_FORMAT(created_at,'%Y-%m') as period, count(*) as count"))
